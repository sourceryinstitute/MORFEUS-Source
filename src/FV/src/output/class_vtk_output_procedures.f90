...
WRITE(output_unit,*) 'BEFORE WRITE_VTK_MESH on image: ',mypnum_()
        CALL write_vtk_mesh (msh, points, cell_ids, v2cconn, icverts, iproc)
WRITE(output_unit,*) 'AFTER WRITE_VTK_MESH on image: ',mypnum_()
WRITE(output_unit,*) 'Before allocate field_vals in write_vtk_morfeus on image: ',mypnum_()
write(output_unit,*) size(sfield), size(cell_ids)
        ALLOCATE (field_vals(1:SIZE(sfield), 1:SIZE(cell_ids)))
        DO i = 1, SIZE(sfield)
          !! This collapses all images to one
!            field_vals(i,:) = out%get_scalar_field(sfield(i))
          !! This keeps things local
            CALL sfield(i)%get_x(scalar_val)
            field_vals(i,:) = scalar_val
        END DO
        IF(mypnum_() /= 0) RETURN ! this need only be done by one processor

WRITE(output_unit,*) 'after allocate field_vals in write_vtk_morfeus on image: ',mypnum_()
...
!            vector_vals(i,1:3,:) = out%get_vector_field(vfield(i))
            ...
            BLOCK
                ...
                ASSOCIATE (my_field => vfield(i))
                  ...
                  DO j = 1, SIZE(x_loc)
!                      CALL x_loc(j)%get_x(scalar_val)
!                      vector_vals(i,1:3,j) = scalar_val
        ...
        END DO
          ...
          END DO
  write(output_unit,*) 'before vtk_mesh%init'
          CALL vtk_mesh%init (points=points, cell_list=morfeus_cells, datatype='double')
                               !! Set up the geometry using a vtk structured grid
  write(output_unit,*) 'before vtk_serial_write for geometry. file_name: ',out%path_()
          CALL vtk_serial_write (geometry=vtk_mesh, filename=TRIM(out%path_()), multiple_io=.FALSE.) !!
  write(output_unit,*) 'after vtk_serial_write for geometry'
          ASSOCIATE (n_scalars => SIZE(fixed_cell_data) + SIZE(sfield), &
              ...
              DO i = 1, SIZE(cell_vals_to_write)
                  ...
                  ELSE
                      ...
                      END ASSOCIATE
                      write(output_unit,*) 'after vectors'
                  END IF
              END DO

          END ASSOCIATE
  write(output_unit,*) 'before vtk_serial_write for celldatasets'
          CALL vtk_serial_write (celldatasets=cell_vals_to_write)  ! pointdatasets=point_vals_to_write
  write(output_unit,*) 'before vtk_serial_write for finishing'
          CALL vtk_serial_write (finished=.TRUE.)
  write(output_unit,*) 'after vtk_serial_write for finishing'
