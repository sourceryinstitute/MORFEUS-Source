...
program test_dimensions
  ...
  !! Test dimensions class behavior
    ...
    use class_dimensions, only : dimensions, operator(==), operator(/=), length_, time_, velocity_, density_, &
    volume_, null_dim_, mass_, energy_

    implicit none

    type(dimensions) :: height_dimensions, displacement_dimensions
    type(dimensions) :: time_step_dimensions, time_dimensions
    type(dimensions) :: velocity_dimensions

    height_dimensions = length_
    displacement_dimensions = length_
    time_step_dimensions = time_
    time_dimensions = time_
    ...
    call assert( mass_ / density_ == ( volume_), "test_dimensions: operator(/)" )
    call assert( (mass_ * velocity_) * velocity_ == ( energy_), "test_dimensions: operator(*)" )
